...
- After an interrupted host runner, never treat one transient ps5debug absence result as authoritative. Relaunch the pinned cleanup ELF, wait for it to settle, repeat the exact-name absence check, and reconcile it with any available operator-visible screen state before another launch.
